...
def model_bert():
    ...
    for epoch in range(num_epoches):
        ...
        for i, (words, labels) in enumerate(train_loader):
            tokens = tokenizer(words, padding=True, truncation=True,
                               max_length=128, return_tensors="pt").to(device)
            input_ids = tokens["input_ids"]
            attention_mask = tokens["attention_mask"]
            ...
            if (i + 1) % 10 == 0:
                print(
                    f"epoch:{epoch + 1}, step:{i + 1}, loss:{total_loss / 10:.4f}")
                total_loss = 0
    ...
    # 保存情感分数
    predict_df["sentiment_score"] = predict_df
    # 保存情感标签
    predict_df["sentiment_label"] = (predict_df > 0.5).astype(int)
