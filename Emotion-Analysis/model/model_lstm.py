...
def model_lstm():
    ...
    wv_input = df_train['text'].map(
        lambda s: [w for w in s.split(" ") if w not in stopwords])

    # 训练 Word2Vec 模型
    word2vec = models.Word2Vec(wv_input, vector_size=64, min_count=1,
                               epochs=1000)
    ...
    class MyDataset(Dataset):
        def __init__(self, df):
            ...
            for s in df["text"].tolist():
                vectors = [word2vec.wv[w] for w in s.split(" ") if
                           w in word2vec.wv.key_to_index]
    ...
    class LSTM(nn.Module):
        ...
        def forward(self, x, lengths):
            ...
            c0 = torch.zeros(self.num_layers * 2, x.size(0),
                             self.hidden_size).to(device)
            packed_input = torch.nn.utils.rnn.pack_padded_sequence(input=x,
                                                                   lengths=lengths,
                                                                   batch_first=True)
    ...
    for epoch in range(num_epoches):
        ...
        for i, (x, labels, lengths) in enumerate(train_loader):
            ...
            if (i + 1) % 10 == 0:
                print(
                    f"epoch:{epoch + 1}, step:{i + 1}, loss:{total_loss / 10}")
                total_loss = 0
    ...
    x, _, lengths = collate_fn(
        list(zip(predict_texts, [-1] * len(predict_texts))))
    x = x.to(device)
    outputs = lstm(x, lengths)
    outputs = outputs.view(-1).tolist()

    # 保存情感分数
    predict_df["sentiment_score"] = outputs
    # 保存情感标签
    predict_df["sentiment_label"] = (outputs > 0.5).astype(int)
